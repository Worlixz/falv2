export const freeCards = [
    {
        nameCollection: "freeCollection1",
        etiquette: "membre inf",
        categorie: "anatomy",
        cards: {
            uodbvqcdnsq: {
                question: "je suis la Q1",
                reponse: true,
                type: "vraiFaux",
                id_card: 'uodbvqcdnsq',
                revisionDate: 0
            }, 
            hvuzpvbe: {
                question: "je suis en q2 FreeCollection",
                reponse: "ceci est la réponse ",
                type: "quiz",
                id_card: "hvuzpvbe",
                revisionDate: 0,
                possibilite: {
                    p1: "ceci est la réponse",
                    p2: "je suis dans p2,",
                    p3: "je suis dans p3,",
                    p4: "je suis dans p4,"
                }
            },
            vbfiylsqfbdcsqil: {
                question: "je suis la Q3",
                reponse: false,
                type: "vraiFaux",
                id_card: 'vbfiylsqfbdcsqil',
                revisionDate: 0
            },
            vfuoiagfce: {
                question: "je suis en q4 FreeCollection",
                reponse: "ceci est la réponse ",
                type: "quiz",
                id_card: "vfuoiagfce",
                revisionDate: 0,
                possibilite: {
                    p1: "je suis en p1",
                    p2: "je suis dans p2,",
                    p3: "ceci est la réponse,",
                    p4: "je suis dans p4,"
                }
            }
        }
    },
    {
        nameCollection: "freeCollection 2",
        etiquette: "membre inf",
        categorie: "physio",
        cards: {
            iulgzvfe: {
                question: "je suis la Q1",
                reponse: true,
                type: "vraiFaux",
                id_card: 'iulgzvfe',
                revisionDate: 0
            }, 
            yfvuizbfd: {
                question: "je suis en q2 FreeCollection 2",
                reponse: "ceci est la réponse FC2",
                type: "quiz",
                id_card: "yfvuizbfd",
                revisionDate: 0,
                possibilite: {
                    p1: "p1",
                    p2: "je suis dans p2,",
                    p3: "je suis dans p3,",
                    p4: "ceci est la réponse FC2"
                }
            },
            oiuytrfdcvbn: {
                question: "je suis la Q3 FC2",
                reponse: false,
                type: "vraiFaux",
                id_card: 'oiuytrfdcvbn',
                revisionDate: 0
            },
            uyetgzhcdsk: {
                question: "je suis en q4 FreeCollection 2",
                reponse: "Ceci est la réponse FC2",
                type: "quiz",
                id_card: "uyetgzhcdsk",
                revisionDate: 0,
                possibilite: {
                    p1: "je suis en p1",
                    p2: "je suis dans p2,",
                    p3: "je suis en p3",
                    p4: "Ceci est la réponse FC2"
                }
            }
        }
    }
]



export const freeCards2 = [
    {
        freeCollection1 : {
            etiquette: "membre inf",
            categorie: "anatomy",
            cards: {
                uodbvqcdnsq: {
                    question: "je suis la Q1",
                    reponse: true,
                    type: "vraiFaux",
                    id_card: 'uodbvqcdnsq',
                    revisionDate: 0
                }, 
                hvuzpvbe: {
                    question: "je suis en q2 FreeCollection",
                    reponse: "ceci est la réponse ",
                    type: "quiz",
                    id_card: "hvuzpvbe",
                    revisionDate: 0,
                    possibilite: {
                        p1: "ceci est la réponse",
                        p2: "je suis dans p2,",
                        p3: "je suis dans p3,",
                        p4: "je suis dans p4,"
                    }
                },
                vbfiylsqfbdcsqil: {
                    question: "je suis la Q3",
                    reponse: false,
                    type: "vraiFaux",
                    id_card: 'vbfiylsqfbdcsqil',
                    revisionDate: 0
                },
                vfuoiagfce: {
                    question: "je suis en q4 FreeCollection",
                    reponse: "ceci est la réponse ",
                    type: "quiz",
                    id_card: "vfuoiagfce",
                    revisionDate: 0,
                    possibilite: {
                        p1: "je suis en p1",
                        p2: "je suis dans p2,",
                        p3: "ceci est la réponse,",
                        p4: "je suis dans p4,"
                    }
                }
            }
        }
    },
    {
        freeCollection2: {
            etiquette: "membre inf",
            categorie: "physio",
            cards: {
                iulgzvfe: {
                    question: "je suis la Q1",
                    reponse: true,
                    type: "vraiFaux",
                    id_card: 'iulgzvfe',
                    revisionDate: 0
                }, 
                yfvuizbfd: {
                    question: "je suis en q2 FreeCollection 2",
                    reponse: "ceci est la réponse FC2",
                    type: "quiz",
                    id_card: "yfvuizbfd",
                    revisionDate: 0,
                    possibilite: {
                        p1: "p1",
                        p2: "je suis dans p2,",
                        p3: "je suis dans p3,",
                        p4: "ceci est la réponse FC2"
                    }
                },
                oiuytrfdcvbn: {
                    question: "je suis la Q3 FC2",
                    reponse: false,
                    type: "vraiFaux",
                    id_card: 'oiuytrfdcvbn',
                    revisionDate: 0
                },
                uyetgzhcdsk: {
                    question: "je suis en q4 FreeCollection 2",
                    reponse: "Ceci est la réponse FC2",
                    type: "quiz",
                    id_card: "uyetgzhcdsk",
                    revisionDate: 0,
                    possibilite: {
                        p1: "je suis en p1",
                        p2: "je suis dans p2,",
                        p3: "je suis en p3",
                        p4: "Ceci est la réponse FC2"
                    }
                }
            }
        }
    }
]